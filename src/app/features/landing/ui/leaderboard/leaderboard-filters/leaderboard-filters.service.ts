        ...
        map(([date, timeframe]) => {
            switch (timeframe) {
                case "last24Hours":
                    return "";
                case "Day":
                    return date ? DateUtils.format(date, "MMMM D, YYYY") : "";
                case "Week":
                    return date
                        ? `${DateUtils.format(DateUtils.getStartOfWeek(date), "MMMM D")} - ${DateUtils.format(DateUtils.getEndOfWeek(date), "MMMM D, YYYY")}`
                        : "";
                case "Month":
                    return date ? DateUtils.format(date, "MMMM D, YYYY") : "";
                default:
