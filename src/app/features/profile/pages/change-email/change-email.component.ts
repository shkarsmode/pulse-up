...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, InputComponent, ProfileLayoutComponent, PrimaryButtonComponent],
...
export class ChangeEmailComponent {
  ...
  public get disabled(): boolean {
    return this.form.invalid || this.submitting || this.emailControl?.value === this.initialEmail;
  }
  ...
  public onSubmit(): void {
    ...
    if (verifyingEmail) {
      this.authenticationService.verifyEmail({
        ...
        continueUrl: window.location.origin + "/profile/verify-email",
      }).subscribe({
        next: () => {
          this.submitting = false;
          this.router.navigateByUrl(`/${AppRoutes.Profile.VERIFY_EMAIL}`, { replaceUrl: true });
        },
        error: (error) => {
          this.submitting = false;
          this.errorMessage = error.message || 'An error occurred while verifying the email. Please try again.';
        }
      })
    } else if (changingEmail) {
      this.authenticationService.changeEmail({
        email: this.emailControl?.value,
        continueUrl: window.location.origin + "/profile/verify-email?showPopup=true",
      }).subscribe({
        next: () => {
          this.submitting = false;
          this.router.navigateByUrl(`/${AppRoutes.Profile.VERIFY_EMAIL}`, { replaceUrl: true });
        ...
        error: (error) => {
          this.submitting = false;
          this.errorMessage = error.message || 'An error occurred while changing the email. Please try again.';
        }
      });
    }
  }
